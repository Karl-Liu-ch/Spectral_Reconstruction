...
if __name__ == '__main__':
    ...
    val_data = ValidDataset(data_root=opt.data_root, crop_size=opt.patch_size, bgr2rgb=True, arg=False, stride=128)
    ...
    for i, (images, labels) in enumerate(val_loader):
        ...
        images = Variable(images)
        labels = Variable(labels)
        output = torch.randn_like(labels)
        mrae = lossmrae(output[:, :, 32:-32, 32:-32], labels[:, :, 32:-32, 32:-32])
        losses_mrae.update(mrae.data)
        print(mrae.data)
    print(losses_mrae.avg)
